...
func (r *productOptionRepository) Create(productOption *domain.ProductOption) error {
	_, err := r.db.NamedExec("INSERT INTO product_option (id, product_id, name, price) VALUES (:id, :product_id, :name, :price)", productOption)
	if err != nil {
		return err
	}
	return nil
...
func (r *productOptionRepository) Update(productOption *domain.ProductOption) error {
	_, err := r.db.NamedExec("UPDATE product_option SET name = :name, price = :price WHERE id = :id", productOption)
	if err != nil {
		return err
	}
	return nil
